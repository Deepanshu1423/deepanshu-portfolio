  ...
  return (
    <nav className="fixed left-0 top-0 z-50 w-full px-4 py-4">
      <div className="mx-auto flex max-w-7xl items-center justify-between rounded-full border border-[var(--accent-light)]/40 bg-[var(--bg-dark)] px-5 py-3 shadow-[0_18px_60px_rgba(43,22,9,0.22)]">
        ...
        >
          <span className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-[var(--accent-light)] to-[var(--accent-main)] text-sm font-black text-[var(--bg-dark)] shadow-lg shadow-[var(--shadow-gold)]">
            DP
          </span>
          Deepanshu Pal
        </a>
          ...
          {navLinks.map((link) => (
            <a
              ...
              className={`rounded-full px-4 py-2 text-sm font-bold transition ${
                activeLink === link
                  ? "bg-[var(--accent-soft)] text-[var(--accent-dark)] shadow-sm"
      ...
      {menuOpen && (
        <div className="mx-auto mt-3 max-w-7xl rounded-3xl border border-[var(--accent-light)]/30 bg-[var(--bg-dark)] p-5 shadow-2xl shadow-[rgba(43,22,9,0.25)] lg:hidden">
